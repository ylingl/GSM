...
public class TsmeMainBaseDAO<T>{
	...
	public enum BaseDao_sortOrder_enum {
		ASC, DESC;
	}
	...
	public int activate(T entity) {
		...
		return namedParameterJdbcTemplate.update(sql.toString(), msps);
	}
	...
	public long getTotalItemsNumBySelectQuery(String sql){
		...
		if(temp[0].equalsIgnoreCase("SELECT")){
			...
			//计算符合条件的总数据条目数
			totalNum = jdbcTemplate.queryForObject(countSql, Long.class);
		}
	...
	@SuppressWarnings("unchecked")
	public boolean oraclePagingFindBySelectQueryAndSortByRowidForSingleTable(String sql, Class<?> entityClass, PageView<T> pageView, Serializable colToSort, BaseDao_sortOrder_enum order){
		String[] temp = sql.split(" ");
		
		long startNum = pageView.getOffset();
		long finishNum = pageView.getFinishRow() + 1;
		
		if(temp[0].equalsIgnoreCase("SELECT") && pageView.getTotalItemsNum() != 0){
			//拼凑分页查询语句
			int k = 0;
			for(int i = 2; i < temp.length; i++){
				if(temp[i].equalsIgnoreCase("FROM")){
			...
			List<?> items = jdbcTemplate.query(querySql, BeanPropertyRowMapper.newInstance(entityClass));
	...
	@SuppressWarnings("unchecked")
	public boolean oraclePagingFindBySelectQueryAndSortByRownumForSingleTable(String sql, Class<?> entityClass, PageView<T> pageView){
		String[] temp = sql.split(" ");
	...
	//
	@SuppressWarnings("unchecked")
	public boolean mysqlPagingFindBySelectQuery(String sql, Class<?> entityClass, PageView<T> pageView) throws Exception {
		
		long offset = pageView.getOffset();
		long rows = pageView.getRows();
		
		String querySql = sql;
		
		querySql = querySql + " LIMIT " + offset + ", " + rows;
