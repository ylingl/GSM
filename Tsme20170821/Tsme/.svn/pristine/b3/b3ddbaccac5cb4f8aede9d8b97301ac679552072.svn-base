...
function hightchart() {
	...
	chart.setOption({
		...
		legend: {
			data:['min','max','一次均值','二次均值','三次均值','四次均值','五次均值','六次均值','原始数据']
		},
		xAxis: {
			type : 'value',
			scale: true
...
function loadJsFiles(){
	...
	if ($('input[name=chartHeight]').val() == "") {
		...
		initRows = $('input[name=initRows]').val();
	}
...
function addseriesIta(obj,objIndex,index,max){
	var allData = [];
	var len = initRows;//obj[objIndex].length;
	for(var i=0; i < len; i++){
	...
	chart.setOption({
		series: [/*{
			...
			data:  formatData(avgArray[12])
		},{
			// 根据名字对应到相应的系列
			name: '二次均值',type: 'line',symbol: 'none',symbolSize:'2',
			data:  formatData(avgArray[13])
		},{
			// 根据名字对应到相应的系列
			name: '三次均值',type: 'line',symbol: 'none',symbolSize:'1',
			data:  formatData(avgArray[14])
		},{
			// 根据名字对应到相应的系列
			name: '四次均值',type: 'line',symbol: 'none',symbolSize:'1',
			data:  formatData(avgArray[15])
		},{
			// 根据名字对应到相应的系列
			name: '五次均值',type: 'line',symbol: 'none',symbolSize:'1',
			data:  formatData(avgArray[16])
		},{
			// 根据名字对应到相应的系列
			name: '六次均值',type: 'line',symbol: 'none',symbolSize:'1',
			data:  formatData(avgArray[17])
		},{
			// 根据名字对应到相应的系列
...
function updateInit(){
	intervalTimer = setInterval(function () {
		jisuan(dataIndex,objFiles[0][dataIndex],avgArray[3],avgRangeArray[0][1]); 
...
<form name="deviceParaForm" id="deviceParaForm" action=""
	onsubmit="submitForm()">
	startFrequency:<input type="text" name="startFrequency" value="94000" /> 
	stopFrequency:<input type="text" name="stopFrequency" value="96000" /> 
	图形高度:<input type="text" name="chartHeight" value="600" /> 
	原始数据行数:<input type="text" name="initRows" value="10" />  
	每行数据点数:<input type="text" name="pointTotal" value="1000" /> 
...
<hr>
<table>
<tr><td><div id="container" style="min-width:1500px;height:500px"></div></td></tr>
<tr><td><div id="alertMsgSpan"></div></td></tr>
</table>
</body>
</html>